...
def is_autograd_numpy(numpy_module):
    return numpy_module.__name__ == 'autograd.numpy'
...
def update_global_stiffness_matrix(K_old: scipy.sparse.csr_matrix, old_voxels: np.ndarray, new_voxels: np.ndarray, Ke: np.ndarray, threshold: float) -> tuple[scipy.sparse.csr_matrix, scipy.sparse.csr_matrix]:
    ...
    # Create delta matrix in CSR format
...
def largest_component(K: scipy.sparse.csr_matrix, F: np.ndarray) -> tuple[scipy.sparse.csr_matrix, np.ndarray]:
    ...
    mask = (components <= 3)
    print(f_mask.shape, mask.shape)
    combined = np.logical_or(f_mask, mask)
    newK = K.copy()
    newF = F.copy()
    newK[~combined, :] = 0
    newK[:, ~combined] = 0
...
class Solver:
    ...
    def __init__(self, K: scipy.sparse.csr_matrix, F: np.ndarray):
        ...
        self.threshold = 1e-8 * np.max(np.abs(K_red.data))
    ...
    def solve(self, K: scipy.sparse.csr_matrix, F: np.ndarray, fixed_nodes, debug: bool = False) -> tuple[np.ndarray, int]:
        ...
        t1 = time.perf_counter()
        K, F = fix_boundary_nodes_fast(np.asarray(fixed_nodes), K, F)
        t2 = time.perf_counter()
